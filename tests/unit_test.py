...
def test_channel_handle(channel_handle):
    assert channel_handle == "MOCK_CHANNEL_HANDLE"
...
def test_dags_integrity(dagbag):
    ...
    for dag_id in expected_dag_ids:
        assert dag_id in loaded_dag_ids, f"DAG '{dag_id}' is not found in the loaded DAGs."

    #3. Check if the number of loaded DAGs is as expected
    assert dagbag.size() == 3
    print("==================")
    print(dagbag.size())

    #4. Check if the number of tasks in each DAG is as expected
